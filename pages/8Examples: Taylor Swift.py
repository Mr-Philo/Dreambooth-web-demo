...
def Taylor_trans_prompt(prompt):
    for str in ["me", "I", "Taylor", "taylor", "Taylor Swift", "Taylor swift"]:
        prompt = prompt.replace(str, "a #4*js! woman")
    return prompt
        
st.set_page_config(layout="wide")
st.title("Some examples from Taylor Swift")

view_train = st.checkbox('View train images (5 of 50 images)')       
if view_train:
    train_dir_name = "./assets/Taylor/train"
...
if chose == 'pre-generated':
    QueryToClass = {
        "A fantasy style portrait painting of a Taylor, smiling clean, shaven round face, rpg dnd, oil painting, unreal, rpg portrait, extremely detailed, artgerm, greg rutkowski greg": "fantasy",
        "A photo of Taylor reading a book": 'book',
        "Cartoon art head portrait of Taylor": 'cartoon',
        "Drawing of Taylor with dramatic light, painted by seb mckinnon and greg rutkowski": 'dramatic',
        "Low poly art head portrait of Taylor": 'low poly',
        "An Oil painting head portrait of Taylor": 'oil',
        "Pencil sketch head portrait of Taylor": 'pencil sketch',
        "Photo of Taylor driving a car": 'car',
        "Photo of Taylor wearing a chef cap": 'chef cap',
        "A fantasy portrait of Taylor as a robot, cyberpunk machine, robed, decorated, intricate intense elegant highly detailed digital painting artstation concept art smooth sharp focus illustration, art by artgerm and greg rutkowski alphonse mucha 8 k": 'robotic',
    }
    option = st.selectbox('Choose the prompt!', sorted(set(QueryToClass)))
    dir_name = os.path.join("./assets/Taylor/result", QueryToClass[option])
    ...
    st.write("We directly use 'Taylor Swift' to guide the original diffusion model generation")
    dir_name = os.path.join("./assets/Taylor/comp_result", QueryToClass[option])
    dir = os.listdir(dir_name)
    column = st.columns(4)
    for i,filename in enumerate(dir):    
        column[i%4].image(os.path.join(dir_name, filename))
    ...
    user_input = st.text_input('Add some prompts! (use \'I\', \'me\', \'Taylor\' to represent Taylor)','A photo of me wearing sunglasses')
    user_input = Taylor_trans_prompt(user_input)
